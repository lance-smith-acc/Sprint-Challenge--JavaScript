...
class Dinosaur {
  constructor(object_name, diet, weight, length, period){
...
const universities = [];
graduates.forEach((currentUni, index) => {
  universities[index] = currentUni.university;
})
universities.sort();
...
const contactInfo = [];
graduates.forEach((currentContact, index) => {
  contactInfo[index] = `${currentContact.first_name} ${currentContact.email}`;
})
...
const unisWithUni = [];
graduates.forEach((currentUni, index) => {
  if (currentUni.university.includes(`Uni`)){
    unisWithUni[index] = currentUni.university;
  }
}) 
...
*/
const displayNames = [];
zooAnimals.forEach((currentAnimal, index) => {
  displayNames[index] = `Name: ${currentAnimal.animal_name}, Scientific:${currentAnimal.scientific_name}`;
})
...
*/
const populationTotal = zooAnimals.reduce(function (accumulator, animalPop){
